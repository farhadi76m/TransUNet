...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label1 = cv2.imread(os.path.join(self.gt_path,"bdd_seg_gt",'val' if self.valid else 'train',image_name.replace('jpg','png')), 0)
        label2 = cv2.imread(os.path.join(self.gt_path,"bdd_lane_gt",'val' if self.valid else 'train',image_name.replace('jpg','png')), 0)
        if not self.valid:
...
class My_batch_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(image_name)
        # print(image_name.replace("batch","labels").replace(".jpg"," (1).png"))
        # print(image_name.replace("batch","ll_labels").replace(".jpg"," (2).png"))
        label1 = cv2.imread(image_name.replace("batch","labels").replace(".jpg"," (1).png"), 0)
        label2 = cv2.imread(image_name.replace("batch","ll_labels").replace(".jpg"," (2).png"), 0)
#        print('laabel1:',label1.shape,'laabel2:',label2.shape)
        if not self.valid:
            if random.random()<0.5:
                combination = (image, label1, label2)
                (image, label1, label2)= random_perspective(
